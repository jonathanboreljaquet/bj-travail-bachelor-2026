...
\begin{table}[H]
    ...
    \hline
        ...
        \texttt{get-matches} & API PadelContext & Récupère la liste des matchs disponibles via le point de terminaison \textbf{<< GET /api/matches >>}. \\ \hline
        \texttt{get-available-slots} & API PadelContext & Récupère la liste des créneaux horaires disponibles via le point de terminaison \textbf{<< GET /api/available-slots >>}. \\ \hline
        \texttt{create-match-from-slot} & API PadelContext & Crée un match à partir d'un créneau horaire libre et y inscrit l'utilisateur authentifié via le point de terminaison \textbf{<< POST /api/matches/from-slot >>}. \\ \hline
        \texttt{join-match} & API PadelContext & Inscrit l'utilisateur authentifié à un match existant via le point de terminaison \textbf{<< POST /api/matches/{matchId}/join >>}. \\ \hline
...
Dans l'architecture du Model Context Protocol, la description sémantique d'une fonctionnalité est critique. Pour comprendre l'objectif et les spécificités d'un outil, les LLMs s'appuient exclusivement sur sa description en langage naturel. Une étude récente, ayant analysé 856 outils MCP, révèle que 97,1 \% d'entre eux contiennent des défauts de conception qualifiés de << Tool Smells >>, avec 56 \% des outils échouant même à définir clairement leur objectif principal (Hasan et al. 2026, p. 4). Les auteurs soulignent que si ces descriptions sont défectueuses, sous-spécifiées ou trompeuses, l'agent IA risque de sélectionner le mauvais outil ou de fournir des arguments invalides, réduisant considérablement la fiabilité du système.
\newline\newline
Bien que l'étude démontre que l'amélioration de ces descriptions permet d'augmenter le taux de succès des tâches, elle met également en garde sur le fait qu'une documentation trop volumineuse augmente le coût et le nombre d'étapes d'exécution. Il est donc nécessaire de trouver un juste équilibre (Hasan et al. 2026, p. 4).
\newline\newline
Pour PadelContext, les outils ont été rigoureusement structurés en s'appuyant sur la grille d'évaluation d'excellence (score de 5/5) proposée par l'étude mentionnée. Chaque description est structurée autour de six piliers sémantiques afin d'atteindre une qualité optimale (Hasan et al. 2026, p. 42) :
\newline\newline
\textbf{1. Purpose (Objectif)} : Explique clairement la fonction, le comportement et les données de retour de l'outil en utilisant un langage précis.
\newline
\textbf{2. Usage Guidelines (Directives d'utilisation)} : Énonce explicitement les cas d'utilisation appropriés et les cas où l'outil ne doit pas être utilisé, incluant une désambiguïsation si le nom de l'outil s'avère ambigu.
\newline
\textbf{3. Limitation (Contraintes et limites)} : Indique clairement ce que l'outil ne retourne pas, les limites de sa portée ainsi que ses contraintes importantes.
\newline
\textbf{4. Parameter Explanation (Explication des paramètres)} : Chaque paramètre est détaillé avec son type, sa signification, son effet sur le comportement de l'outil, et son statut (requis ou valeur par défaut).
\newline
\textbf{5. Examples vs. Description Balance (Équilibre Exemples/Description)} : La description textuelle se suffit à elle-même ; les exemples fournis servent uniquement à compléter l'explication et non à la remplacer.
\newline
\textbf{6. Length and Completeness (Exhaustivité)} : La description de l'outil est composée de quatre phrases ou plus, rédigées dans un style concis et bien structuré, couvrant tous les aspects de son utilisation.
\newline\newline
