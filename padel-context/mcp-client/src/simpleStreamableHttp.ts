// Source : https://github.com/modelcontextprotocol/typescript-sdk/blob/main/examples/client/src/simpleStreamableHttp.ts
...
function printHelp(): void {
  ...
  console.log(
    "  collect-info-task [type]   - Test bidirectional task support (server+client tasks) with elicitation",
  );
  console.log(
...
async function connect(url?: string): Promise<void> {
  ...
  try {
    ...
    client.setRequestHandler("elicitation/create", async (request, extra) => {
      ...
      console.log(
        `Task Creation Requested: ${request.params.task ? "yes" : "no"}`,
      );
      ...
      const returnResult = async (result: {
        ...
        if (request.params.task && extra.task?.store) {
          ...
          await extra.task.store.storeTaskResult(
            task.taskId,
            "completed",
            result,
          );
        ...
        for (const [fieldName, fieldSchema] of Object.entries(properties)) {
          ...
          try {
            if (answer === "" && field.default !== undefined) {
              content[fieldName] = field.default as
                | string
                | number
                | boolean
                | string[];
              ...
              content[fieldName] = parsedValue as
                | string
                | number
                | boolean
                | string[];
...
async function callCollectInfoWithTask(infoType: string): Promise<void> {
  ...
  console.log(
    "This will create a task on the server, which will elicit input and create a task on the client.\n",
  );
