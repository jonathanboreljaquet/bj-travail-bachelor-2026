...
describe("GET /matches (query-based)", () => {
  ...
  it("returns matches filtered with complete MCP criteria", async () => {
    ...
    expect(matches.some((match: any) => match.id === createdMatchId)).toBe(true);
    ...
    matches.forEach((match: any) => {
      ...
      //Club informations
      expect(match).toHaveProperty("club");
      expect(match.club).toHaveProperty("name");
      expect(match.club).toHaveProperty("city", "Lancy");
      expect(match.club).toHaveProperty("openingTime");
      expect(match.club).toHaveProperty("closingTime");
      //Court informations
      ...
      match.participants.forEach((participant: any) => {
        ...
        expect(participant).toHaveProperty("level");
        expect(participant).not.toHaveProperty("id");
        expect(participant).not.toHaveProperty("password");
        expect(participant).not.toHaveProperty("createdAt");
      });
    });
  });
});
